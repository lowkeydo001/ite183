...
class Employees(APIView):
    def get(self, request):  # Request - kung unsa ang data sa frontend, ayha ra maggamit ang request pag mag post
        employees = Employee.objects.all()  # E fetch ang data sa tanan employees
        serializer = EmployeeSerializer(employees, many=True)  # Para ang data nga gi return sa Employee.objects.all (data) convert into json.. Ang gi serialize nga data e response sa client dayon
